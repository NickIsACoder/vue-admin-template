<template>
  <div class="group">
    <el-form ref="input_form" :model="input_form" label-width="100px" class="demo-ruleForm">
      ...
      <el-form-item v-for="(item, index) in input_form.options" :key="index.serial" label="" prop="options" style="margin-bottom:0px">
  ...
  methods: {
    ...
    submitForm(formName) {
      ...
      this.$refs[formName].validate((valid) => {
        if (valid) {
          this.$emit('toSaveData', this.input_form, 'groupInput', 4)
        } else {
    ...
    cancelForm() {
      this.$emit('toCancel', 'groupInput', 4)
    }
  }
}
</script>
